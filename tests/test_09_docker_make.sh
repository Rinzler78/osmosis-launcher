...
ROOT_DIR="$(mktemp -d "$ROOT_DIR_BASE/test_09_docker_make.XXXXXX")"
...
if [ "$ALL_PLATFORMS" = false ]; then
  ...
  if ! (
    cd "$WORK_DIR"
    bash "$DOCKER_MAKE_SH" --os "$GO_OS" --arch "$GO_ARCH"
  ); then
    fail "docker_make.sh failed"
  fi
  BINARY_PATH="$WORK_DIR/osmosisd"
  ...
  for os in $(jq -r '.os[]' "$SUPPORTED_JSON"); do
    for arch in $(jq -r ".platforms[\"$os\"][]" "$SUPPORTED_JSON"); do
      ...
      if (
        cd "$WORK_DIR"
        bash "$DOCKER_MAKE_SH" --os "$os" --arch "$arch"
      ); then
        if [ -f "$WORK_DIR/osmosisd" ]; then
          mv "$WORK_DIR/osmosisd" "$WORK_DIR/osmosisd-${os}-${arch}"
